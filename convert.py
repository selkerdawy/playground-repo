...
def convert(model, layer_from, layer_to, index_start=0, index_end=-1, index=0, **kw_args):
    if index_end < 0:
    ...
    for name, module in model._modules.items():
        if isinstance(module, (layer_from)):
            if index >= index_start and index <= index_end:
                model._modules[name] = layer_to(module, **kw_args)
            index += 1
        
        if len(list(module.children())) > 0:
            # recurse
            model._modules[name], index = convert(module, layer_from, layer_to, index_start, index_end, index, **kw_args)
    return model, index
